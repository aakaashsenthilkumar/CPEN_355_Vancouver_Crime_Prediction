...
agg["safety_score"] = 100 * (
    1 - (agg["weighted_score"] - min_ws) / (max_ws - min_ws)
)
...
# ── 7. DEFINE FEATURES ──────────────────────────────────────
# Uses only crime characteristics — no leakage from the target
# weighted_score is excluded because safety_score is derived directly from it
...
# ── 8. TIME-BASED SPLIT (train: 2003-2019, test: 2020+) ─────
...
joblib.dump(best_model_obj, "model/best_model.pkl")
